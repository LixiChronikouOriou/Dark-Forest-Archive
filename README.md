# Dark Forest Archive

Collection of inactive (not touched for years) or abandoned (declared as such or deleted repository) projects which introduce material for new species. Archived for inspiration and hopefully some kind of revival.

The difference to the races section of the [Plugin Archive](https://github.com/Hecter94/EndlessSky-PluginArchive) is that it also includes archaeological material which was never published.

There might be interesting species out there in the wild. Feel free to contact me via issue or PR here on Github, or in the corresponding [Discord channel](https://discord.com/channels/251118043411775489/1432671335623954472) if you see something interesting to preserve.

**Thanks**: 1010todd and The_Legendary_M for research help, and of course all authors.

| Last Update | Name                | Author                           | Status    |
|-------------|---------------------|----------------------------------|-----------|
| 2016-08-17  | COTSA               | Hondolor                         | inactive  |
| 2017-02-02  | The Forgotten       | Startome                         | inactive  |
| 2017-07-14  | Trin<sup>1</sup>    | Elyssaen                         | inactive  |
| 2019-02-02  | Terrans             | TimCabbage                       | inactive  |
| 2019-02-25  | Octyl               | Ferociousfeind                   | inactive  |
| 2020-01-12  | Eternals            | Comnom                           | abandoned |
| 2020-06-13  | *no name*           | Loyse                            | abandoned |
| 2021-03-20  | Akasha              | jchamlin, then EricD112          | abandoned |
| 2021-04-26  | Gulliver            | Test-711                         | inactive  |
| 2021-05-14  | Galactic Couriers   | 10010101001                      | inactive  |
| 2021-06-13  | Abyssals            | Arachi                           | abandoned |
| 2021-07-12  | Asheyakan           | Nams                             | abandoned |
| 2022-07-31  | The Enclave         | Storm-Crow01, patch: 1010todd    | inactive  |
| 2022-10-06  | Ertluk              | ZBok                             | abandoned |
| 2022-10-20  | Binary reOrder      | Vaelophis Nyx, then Kezethun     | abandoned |
| 2022-10-22  | Shirni              | Quantumshark                     | abandoned |
| 2022-12-08  | Altera<sup>2</sup>  | EricD112                         | inactive  |
| 2023-01-07  | Exporianes          | Kestrel1110                      | inactive  |
| 2023-04-06  | Lupainian           | 3agleEmpire                      | abandoned |
| 2023-05-16  | Mata                | Karirawri                        | inactive  |
| 2023-07-05  | Adamas              | toilethinges, patch: Kestrel1110 | inactive  |
| 2023-07-27  | AES Aben            | DarcyManoel                      | abandoned | 
| 2023-07-27  | AES Fallen          | DarcyManoel                      | abandoned | 
| 2024-09-12  | Searchers<sup>2</sup> | mOctave                          | inactive  |
| 2025-02-22  | Xapleaux<sup>2</sup> | MasterOfGrey                     | inactive  |
| 2025-10-23  | Irm                 | DarcyManoel                      | abandoned |

(1) Blender and Gimp files for the Trin can be found in the mirrored [assets repo](https://github.com/LixiChronikouOriou/trinAssets).<br>
(2) The author has not totally lost hope to continue one day.

Currently revival process is more or less ongoing for the material of:<br>
Altera, Eternals, (no name), Ertluk, Mata, Trin

For further projects which will be included in the next time see `to add.txt`.
